...
class ABSALightningModule(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss = self.loss(logits.view(-1, self.conf.model.num_classes), labels.view(-1))
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # distribution on each label so have to take argmax
        predictions = logits.max(dim=-1)
        acc = self.acc_metric(predictions, labels.view(-1))
        acc = acc.cpu().item()
    ...
    def test_step(self, batch, batch_idx):
        ...
        self.testing_step_outputs.append(
            {"acc": acc, "f1": f_score, "precision": p_score, "recall": r_score}
        )

        print(
            f"Acc - MulticlassAcc: {acc} Precision: {p_score} Recall: {r_score} F1: {f_score}"
        )
        return acc
    ...
    def configure_optimizers(self):
        optimizer = AdamW(
            self.parameters(), lr=self.conf.model.train.lr, weight_decay=0.3
        )
    ...
    def setup_train_dataloader(self):
        train_ds = ABSADataset(
            tokenizer=self.tokenizer, path=self.conf.model.train.train_dir
        )
        dev_ds = ABSADataset(
            tokenizer=self.tokenizer, path=self.conf.model.train.dev_dir
        )
        train_dataloader = ABSADataloader(
            train_ds,
            batch_size=self.conf.model.train.batch_size,
            shuffle=True,
            tokenizer=self.tokenizer,
        )
        dev_dataloader = ABSADataloader(
            dev_ds,
            batch_size=self.conf.model.train.batch_size,
            shuffle=True,
            tokenizer=self.tokenizer,
    ...
    def setup_test_dataloader(self):
        test_ds = ABSADataset(
            tokenizer=self.tokenizer, path=self.conf.model.train.test_dir
        )
        test_dataloader = ABSADataloader(
            test_ds,
            batch_size=self.conf.model.train.batch_size,
            shuffle=False,
            tokenizer=self.tokenizer,
